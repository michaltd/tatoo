...
public abstract class DBConnection {
  ...
  public enum DBases {
    ...
    private DBases(String driverString, String connectionString,
  ...
  public final Dataset read(Class<?> c, long id) {
    if (connect()) {
      LinkedList<Dataset> results = createQuery().get(c)
          .addCondition("\"dataset\".dataset_id=" + id).execute();
  ...
  public void migrate(VersionNumber toVersion) {
    ...
    try {
      ...
      LinkedList<Dataset> sqlResults = createQuery().get(VersionNumber.class)
  ...
  private void migrate_up(LinkedList<Migration> migrations) {
    for (Migration m : migrations) {
      if (m.getVersion().compareTo(Tatoo.VERSION) > 0)
        throw new MigrationInvocationException(
            "VersionNumber of Migration less than actual VersionNumber");
      m.migration_up();
    }
  }
