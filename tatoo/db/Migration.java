...
/**
 * Eine Migration überführt eine Datenstruktur auf die sie angewendet wird in eine neue Version. Dabei kann es sich
 * sowohl um eine niedrigere Version als auch eine höhere handeln. Je nach Aufgabe wird eine der beiden Methoden
 * {@link Migration#up()} oder {@link Migration#down()} aufgerufen.<br /> Eine Migration wird von einer konkreten
 * Migration implementiert, indem die Methoden up(), down() und getVersion() implementiert werden. Diese beiden Methoden
 * sollten die Migrations-Methoden createTable, alterTable und dropTable aufrufen. Es kann jedoch beliebiger Code
 * eingefügt werden.<br /> Eine Migratin muss sicherstellen, dass die Datenstruktur nach dem Aufruf von up() und down()
 * - oder andersherum erst down() dann up() - die selbe Struktur hat wie vor der Ausführung. Werden z.B. Datensätze bei
 * up() hinzugefügt, müssen diese mit down() wieder entfernt werden.
 *
 ...
 * }
 *
 * } </pre> Eine Migration verfügt selbst über eine {@link VersionNumber}. Diese wird mit der Migration in die
 * Datenstruktur geschrieben. So kann nachvollzogen werden welche Migrationen schon ausgeführt wurden und welche nicht.
 *
 * @author mkortz
 *
 */
public abstract class Migration implements Comparable<Migration> {
    ...
    /**
     * Erzeugt eine Tabelle mit einer Automatisch hochzählenden ID. Die ID hat immer den Namen der Tabelle mit einem
     * angehängten "_id".
     *
     * @param name Der Name der zu erzeugenden Tabelle.
     * @param columns Die Spalten der Tabelle. Die Spalten müssen in der Form "spaltenname:spaltentyp" übergeben werden,
     * ansonsten wird eine {@link DataDefinitionException} geworfen.
     ...
     *
     * @param name Der Name der Tabelle welche gelöscht werden soll.
     */
    ...
    /**
     * führt die Methode
     * <code>down()</code> aus und löscht die Versionsnummer aus der Datenstruktur.
     */
    ...
    /**
     * führt die Methode
     * <code>up()</code> aus und fügt die Versionsnummer in die Datenstruktur ein.
     */
    // TODO: prüfen ob die migration überhaupt erlaubt ist. Beispiel: es sind noch
    // Migrationen vor dieser durchzuführen
