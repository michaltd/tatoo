...
public class ConditionParser
{
     ...
     * Der Wurzelknoten
     */
    private final ParseNode rootEntity;
    ...
    private void buildParseTree(AbstractEntity entity)
    {
        ...
        if (entity.hasChilds()) {
            //@TODO there probably should happen something here;
        }
    }
     ...
     * aus dem String.
     *
     * @param infix Infixterm mit Conditdions als Operanden.
     ...
     *
     * @param term Der Term, der den Ort der Condition beschreibt.
     ...
     * Terms.
     *
     * @param infixExpr der Term in Infixform
        ...
        for (int i = 0; i < infixExpr.length(); i++) {
            ...
            if (isOperator(c)) {
                if (resultString.length() > 0) {
                    resultArr.add(resultString.toString().trim());
    ...
    {
        return operators.contains(val) && val.length() > 0;
    }

    private boolean isParenthesis(String val)
    {
        return parenthesis.contains(val) && val.length() > 0;
    }
    ...
    private static class ParseNode
    {
        ...
        HashMap<String, ParseNode> childs = new HashMap<>();
